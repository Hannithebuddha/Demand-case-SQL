...
--
-- Этот блок является exploratory analysis исходного факта.
-- Здесь исключаем только NULL, чтобы нулевые/отрицательные
-- значения оставались видимыми после Data Quality проверки.
-- В финальных бизнес-KPI (03-05) используется clean_sales:
...
-- YoY не рассчитываем через LAG: получаем годовые агрегаты,
-- которые затем сравниваются в финальном аналитическом блоке.
SELECT
...
LEFT JOIN products p ON s.product_id = p.product_id
WHERE s.quantity IS NOT NULL
GROUP BY EXTRACT(YEAR FROM s.sale_date), p.product_id, p.product_name, p.category
ORDER BY p.product_id, year;

-- 9. Новые продукты
SELECT product_id, product_name, category, launch_date, status
FROM products
WHERE launch_date >= DATE '2025-01-01'
ORDER BY launch_date, category, product_id;
...
LEFT JOIN customers c ON s.customer_id = c.customer_id
WHERE s.quantity IS NOT NULL
GROUP BY EXTRACT(YEAR FROM s.sale_date), c.customer_id, c.customer_name, c.channel
